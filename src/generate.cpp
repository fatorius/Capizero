...
// special function to generate moves in case of check
vector<string> generateWithCheck(Board b, Square s, piece p, side t){
    vector<string> array;
    direction pin = pinnedDirection(b, s);
    ...
    if (!isCheck(b)){  
        ...
        for (int_fast8_t x = 0; x < 8; x++){
            for (int_fast8_t y = 0; y < 8; y++){
                ...
                vector<string> tempMoves = generateWithCheck(b, s, p, t);
                for (unsigned int i = 0; i < tempMoves.size(); i++){
                    moves.push_back(tempMoves[i]);
                }
            }
